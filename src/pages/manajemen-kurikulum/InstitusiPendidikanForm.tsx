...
const InstitusiPendidikanForm: React.FC<InstitusiPendidikanFormProps> = ({ initialData, onSuccess, onCancel }) => {
  ...
  const { data: usersData } = useGetTeachersQuery(); // Menggunakan useGetTeachersQuery untuk mendapatkan data user dengan staff
  ...
  // Ekstrak data staf dari usersData
  const staffs = useMemo(() => {
    if (!usersData?.data) return [];
    return usersData.data
      .filter(user => user.staff) // Hanya user yang memiliki staff
      .map(user => user.staff) as Staff[]; // Ekstrak properti staff
  }, [usersData]);
