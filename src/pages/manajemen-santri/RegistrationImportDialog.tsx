...
const RegistrationImportDialog: React.FC<Props> = ({ open, onOpenChange, onImported }) => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const resp = await fetch(`${base}main/registration/import/template`, {
        method: 'GET',
        headers: {
      ...
      a.download = matched?.[1] || 'template-pendaftaran-santri-baru.xlsx';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast.showSuccess('Template pendaftaran berhasil diunduh.');
    } catch (err) {
      console.error(err);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${base}main/registration/import`, {
        method: 'POST',
        headers: {
          ...getAuthHeader(),
        } as any,
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!isUploading && !isDownloading) onOpenChange(v); }}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Impor Pendaftaran Santri Baru</DialogTitle>
          <DialogDescription>Unggah file Excel (.xlsx) atau CSV sesuai template yang disediakan untuk pendaftaran calon santri baru.</DialogDescription>
        </DialogHeader>
...
export default RegistrationImportDialog;
