...
const SantriImportDialog: React.FC<Props> = ({ open, onOpenChange, onImported }) => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const resp = await fetch(`${base}main/student/import/template`, {
        method: 'GET',
        headers: {
      ...
      a.download = matched?.[1] || 'template-santri.xlsx';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${base}main/student/import`, {
        method: 'POST',
        headers: {
          ...getAuthHeader(),
        } as any,
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!isUploading && !isDownloading) onOpenChange(v); }}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Impor Santri</DialogTitle>
