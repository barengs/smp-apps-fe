...
interface StudentFormStepProps {
  ...
  onBack: () => void;
  onNext: (data: StudentFormValues) => void; // Changed to onNext for wizard flow
  isSubmitting: boolean;
}

const StudentFormStep: React.FC<StudentFormStepProps> = ({ initialData, onBack, onNext, isSubmitting }) => {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea placeholder="Alamat lengkap santri..." {...field} value={field.value || ''} className="min-h-[80px]" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
