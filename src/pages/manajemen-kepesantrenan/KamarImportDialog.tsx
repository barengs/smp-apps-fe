...
import { useImportRoomsMutation, useDownloadRoomTemplateMutation } from '@/store/slices/roomApi';
import { showSuccess, showError, showLoading, dismissToast } from '@/utils/toast';
import { FetchBaseQueryError } from '@reduxjs/toolkit/query';
import { Download } from 'lucide-react';

interface KamarImportDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

const KamarImportDialog: React.FC<KamarImportDialogProps> = ({ isOpen, onClose }) => {
  ...
  const [importRooms, { isLoading }] = useImportRoomsMutation();
  const [downloadTemplate, { isLoading: isDownloading }] = useDownloadRoomTemplateMutation();
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', 'template_import_kamar.xlsx');
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await importRooms(formData).unwrap();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Impor Data Kamar</DialogTitle>
          <DialogDescription>
            Pilih file Excel untuk mengimpor data kamar secara massal. Pastikan format file sesuai dengan template yang disediakan.
          </DialogDescription>
        </DialogHeader>
          ...
          {!importResult ? (
            <>
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label htmlFor="room-file">File Excel (.xlsx, .xls, .csv)</Label>
                <Input 
                  id="room-file" 
                  type="file" 
                  accept=".xlsx, .xls, .csv" 
...
export default KamarImportDialog;
