...
import { useUpdateParentPhotoMutation } from '@/store/slices/parentApi';
import * as toast from '@/utils/toast';
import { compressImage } from '@/utils/imageCompression';
import Webcam from 'react-webcam';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

interface ParentPhotoUploadDialogProps {
    open: boolean;
...
const ParentPhotoUploadDialog: React.FC<ParentPhotoUploadDialogProps> = ({
    open,
    onOpenChange,
    parentId,
    currentPhotoUrl,
    ...
    const [updatePhoto, { isLoading }] = useUpdateParentPhotoMutation();
                ...
                compressImage(f, { maxSizeKB: 2048, maxWidth: 1600, maxHeight: 1600, quality: 0.9 })
                    .then((compressed) => {
                        ...
                        if (newSizeKB > 2048) {
                            ...
                            toast.showSuccess(`Foto dikompresi: ${Math.round(newSizeKB / 1024)} MB.`);
                            setFile(compressed);
                        }
                    })
                    .catch(() => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await updatePhoto({ id: parentId, photo: file }).unwrap();
            toast.showSuccess('Foto wali santri berhasil diperbarui.');
            onOpenChange(false);
            setFile(null);
            setCameraImage(null);
            if (onUploaded) onUploaded();
        } catch (err: any) {
            toast.showError(err?.data?.message || 'Gagal mengunggah foto. Silakan coba lagi.');
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle>Perbarui Foto Wali Santri</DialogTitle>
                </DialogHeader>
                ...
                <Tabs defaultValue="upload" className="w-full">
                    ...
                    <TabsContent value="upload" className="space-y-4 p-4 border rounded-md mt-2">
                        ...
                        <div className="flex gap-2 justify-center">
                            <input type="file" accept="image/*" onChange={handleFileChange} className="hidden" id="parent-photo-input" />
                            <Button variant="outline" type="button" onClick={() => document.getElementById('parent-photo-input')?.click()}>
...
export default ParentPhotoUploadDialog;
